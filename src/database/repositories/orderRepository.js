...
class OrderRepository {
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new Order().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Order().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Order().collectionName);
  ...
  async populate(record) {
    ...
    record.products =
      (await FirebaseRepository.findRelation('product', record.products)) ||
      [];
  ...
  async _createAuditLog(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Order().modelName,
        entityId: id,
        action,
        values: data,
      },
